...
## Repositorio Central de Datos para Vectopen
## Gestiona proyecto, sesión, shapes, capas, artboards y undo/redo
...
var undo_redo: UndoRedoManager = null # Sistema de historial
...
func _ready() -> void:
	...
	if not GlobalEvents:
		push_warning("DataRepository: GlobalEvents no encontrado. Las señales no se emitirán.")
	...
	print("DataRepository: Inicializado correctamente")
...
func _check_for_recovery() -> void:
	...
	if FileAccess.file_exists(recovery_path):
		...
		if data and data.get("timestamp", 0) > Time.get_unix_time_from_system() - 86400:  # Último día
			print("DataRepository: Existe sesión de recuperación del día anterior")
...
func new_project(project_name: String = "Nuevo Proyecto") -> void:
	...
	print("DataRepository: Nuevo proyecto creado - ", project_name)
...
func save_project(path: String = "") -> String:
	if not project:
		push_error("DataRepository: No hay proyecto activo para guardar")
		return ""
	
	var save_path = path if not path.is_empty() else project.file_path
	
	if save_path.is_empty():
		push_error("DataRepository: No se especificó ruta para guardar")
		return ""
	...
	if success:
		...
		print("DataRepository: Proyecto guardado en - ", save_path)
		return save_path
...
func _save_to_path(path: String) -> bool:
	...
	var file = FileAccess.open(path, FileAccess.WRITE)
	if not file:
		push_error("DataRepository: No se pudo crear archivo - ", path)
		return false
...
func load_project(path: String) -> bool:
	if not FileAccess.file_exists(path):
		push_error("DataRepository: Archivo no existe - ", path)
		return false
	...
	if not data:
		push_error("DataRepository: Error al parsear JSON")
		return false
	...
	print("DataRepository: Proyecto cargado - ", project.name)
	return true
...
func update_shape(shape_id: String, property: String, value) -> void:
	...
	if not shape:
		push_error("DataRepository: Shape no encontrado - ", shape_id)
		return
...
func _save_incremental(path: String) -> bool:
	...
	var file = FileAccess.open(path, FileAccess.WRITE)
	if not file:
		push_error("DataRepository: No se pudo crear archivo incremental - ", path)
		return false
...
class SessionData:
	...
	var clipboard_shapes: Array[ShapeData] = []
