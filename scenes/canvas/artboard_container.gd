...
var current_tool_instance: Object = null # Aquí guardas la instancia de MoveTool cuando se activa
...
func set_active_tool(tool_instance: Object) -> void:
...
func _process(_delta: float) -> void:
	...
	if current_bounding_box and current_tool_instance and current_tool_instance.has_method("get_class_name") and current_tool_instance.selected_shapes.size() > 0:
		...
		# Obtenemos el rectángulo macro que une todas las figuras (estilo profesional)
		...
		current_bounding_box.update_overlay(
			macro,
			current_tool_instance.current_key_mode,
